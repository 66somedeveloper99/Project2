...
public class Commander {
    ...
    public boolean hasNextCommand() { return currentCommandIndex < commands.length; }
    ...
    private void Execute(ICommand command) {
        ...
        * for example, if command is a DownCommand, it should call drawer.Down() like below
        
        if (command.getClass().equals(DownCommand.class)) {
             drawer.Down();
        }

        * if the method requires variables, we should get the variables from the command with casting 
        * for example, in the below example we're checking if the command is SizeCommand. if it is, we get it's size
